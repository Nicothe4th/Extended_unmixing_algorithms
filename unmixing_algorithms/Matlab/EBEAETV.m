function [P,A,W,S,Yh,Ji]=EBEAETV(Y,N,parameters,Po,oae)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% [P,A,W,S,Yh,Ji]=EBEAETV(Y,n,parameters,Po,oae)
%
% Estimation by Extended Blind End-member and Abundance Extraction 
% Total Variation and Linear Mixture Model 
%
...
%   parameters --> 11x1 vector of hyper-parameters in BEAE methodology
%              = [initicond rho lambdaTV  tau nu nRow nCol epsilon maxiter  ...
%                      parallel display]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Default hyper-parameters of EBEAE-TV algorithm
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global NUMERROR
...
if display==1
        disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
        disp('EBEAE with Total Variance');
...
while (Jp-J)/Jp >= epsilon && iter < maxiter && oae==0 && NUMERROR==0
    ...
    A = abundance(Ym,P,W,lambda,parallel);
    W = totalVariance(A,Ym,P,lambda,nu,tau,nRow,nCol,epsilon,maxiter); 
    Pp = P;
    if NUMERROR==0
        P = endmember(Ym,A,rho); 
    end
    Jp=J;
    J=norm(Ym-P*A,'fro'); Ji(iter)=J;
    if J > Jp
        P=Pp; break;
    end
    if display ==1
...
function A = abundance(Y,P,W,lambda,parallel)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% A = abundance(Y,P,W,V,parallel,lambda)
%
...
%   lambda          = regularization term of spatial coherence \in [0,1]
%
% Output Argument
...
%
% Juan Nicolas Mendoza-Chavaria & Daniel Ulises Campos Delgado
% FC-UASLP & ULPGC
% Version: May/2024
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
