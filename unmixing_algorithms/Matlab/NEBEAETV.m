function [P,A,W,Ds,S,Yh, Ji]=NEBEAETV(Y,N,parameters,Po,oae)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
while (Jp-J)/Jp >= epsilon && iter <= maxiter && oae==0 && NUMERROR==0
    ...
    Jp=J;
    residual = Ym - (repmat((1-D)',[L,1]).*(P*A)) - ( repmat(D',[L,1]).*((P*A).*Y));
    J=norm(residual,'fro');
...
[N,K] = size(A);
L = size(Y,1);
R = sum(N - (1:N-1)); % Match Python summation logic
em = eye(N);
...
O = N * em - ones(N,N);
GradP = GradP / K + rho * (Po * O) / R;  % Ensure correct division
...
P_est(~isfinite(P_est) | P_est < 0) = 0;  
