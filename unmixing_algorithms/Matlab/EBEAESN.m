function [P,A,S,Yh,V,Ji]=EBEAESN(Yo,N,parameters,Po,oae)
...
if display==1
        ...
        elseif oae==0 && initcond==7
            disp('Po is constructed based on Simplex Volume Maximization by Chan et al. (2011)');
        elseif oae==0 && initcond==8
            disp('Po is constructed based on Simplex identification via split augmented Lagrangian by Bioucas-Dias (2009)');
        end
end
...
while (Jp-J)/Jp >= epsilon && iter < maxiter && oae==0 && NUMERROR==0
    ...
    Jp=J;
    J=norm(Ym-P*Am-Vm,'fro'); Ji(iter)=J;
    if J > Jp
        P=Pp; break;
    end
    if display==1
...
if NUMERROR==0
    ...
    if oae==1  
        ...
        while (Jp-J)/Jp >= epsilon && iter < maxiter && NUMERROR==0
            ...
            Jp=J;
            J=norm(Ymo-P*A-V,'fro'); Ji(iter)=J;
            iter=iter+1;
        end
