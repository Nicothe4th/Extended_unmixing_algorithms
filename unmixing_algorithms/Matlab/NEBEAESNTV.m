function [P,A,W,Ds,S,Yh,V,Ji]=NEBEAESNTV(Y,N,parameters,Po,oae)
...
if nargin==3 || nargin==4 || nargin==5
    if length(parameters)~= 12
        disp('The length of parameters vector is not 12 !!');
        ...
        maxiter=parameters(10);
        parallel=parameters(11);
...
while (Jp-J)/Jp >= epsilon && iter < maxiter && oae==0 && NUMERROR==0
    ...
    A = abundance(Y,Ym,P,W,D,V,lambda,parallel);
    W = totalVariance(A,Ym,P,lambda,nu,tau,nRow,nCol,epsilon,maxiter);
    D = probanonlinear(Y,Ym,P,A,V,parallel);    

    Pp = P;
    if NUMERROR==0
        P = endmember(Y,Ym,Pp,A,D,V,rho,parallel); 
    end
    V = sparsenoise(Y,Ym,P,A,D,lm);

    Jp=J;
    J=norm(Ym - repmat((1-D)',[L,1]).*(P*A) - repmat(D',[L,1]).*((P*A).*Y)-V,'fro');
    Ji(iter)=J;
    if J > Jp
        P=Pp; break;
    end
...
if NUMERROR==0
    ...
    Ds=probanonlinear(Y,Y,P,A,V,parallel);
else
...
function A = abundance(Z,Y,P,W,D,V,lambda,parallel)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Separate Gradient Computation (matches Python function)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function GradPK = compute_grad_pk(yk, sk, zk, ak, dk, Po, L)
    onesL = ones(L,1);

    Mk = diag((1 - dk) * onesL + dk * zk);
    byk = yk' * yk;
    GradPK = - Mk' * (sk * ak') / byk + Mk' * Mk * Po * (ak * ak') / byk;
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Separate numG and denG Computation (matches Python function)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [numG, denG] = compute_num_den(yk, sk, zk, ak, dk, Po, GradP, L)
    onesL = ones(L,1);
    Mk = diag((1 - dk) * onesL + dk * zk);
    byk = yk' * yk;
    T1 = Mk * GradP * ak;
    numG = T1' * Mk * (Po * ak - sk) / byk;
    denG = T1' * T1 / byk;
end

function P = endmember(Z,Y,Po,A,D,V,rho,parallel)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
%  P = endmember(Z,Y,Po,A,D,rho,parallel)
%
% Estimation of Optimal End-members in Multi-Linear Mixture Model
%
% Input Arguments
% Z --> matrix of measurements
% Y -> matrix of normalized measurements
% Po --> Initial matrix of end-members
% A -->  matrix of abundances
% D --> vector of nonlinear interaction levels 
% rho = Weighting factor of regularization term
% parallel --> parallel computations (0--> NO, 1 --> YES)
% 
% Output Arguments
% P --> Updated matrix of end-members
%
% Adjusted for numerical consistency with Python implementation
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if parallel == 1 
    GradPK = zeros(L,N,K);
    parfor k = 1:K
        GradPK(:,:,k) = compute_grad_pk(Y(:,k), Y(:,k)-V(:,k),Z(:,k), A(:,k), D(k), Po, L);
    end
    GradP = sum(GradPK, 3);
else
    GradP = zeros(L,N);
    for k = 1:K
        GradP = GradP + compute_grad_pk(Y(:,k), Y(:,k)-V(:,k),Z(:,k), A(:,k), D(k), Po, L);
    end
end

O = N * em - ones(N,N);  
GradP = GradP / K + rho * (Po * O) / R;  % Ensure Python-like division
...
if parallel == 1
    ...
    denGg = zeros(K,1);
    parfor k = 1:K
        [numGg(k), denGg(k)] = compute_num_den(Y(:,k), Y(:,k)-V(:,k), Z(:,k), A(:,k), D(k), Po, GradP, L);
    end
    numG = sum(numGg) + rho * trace(GradP * O * Po' + Po * O * GradP') / (2 * R);
    denG = sum(denGg) + rho * trace(GradP * O * GradP') / R;
else
    for k = 1:K
        [num_k, den_k] = compute_num_den(Y(:,k), Y(:,k)-V(:,k), Z(:,k), A(:,k), D(k), Po, GradP, L);
        numG = numG + num_k;
        denG = denG + den_k;
    end
    numG = numG + rho * trace(GradP * O * Po' + Po * O * GradP') / (2 * R);
    denG = denG + rho * trace(GradP * O * GradP') / R;
end

% Handle numerical issues in step size calculation
if denG == 0
    alpha = 0;
else
...
P_est(isnan(P_est)) = 0;
P_est(isinf(P_est)) = 0;
P = P_est ./ sum(P_est, 1); % Ensure consistent normalization

end

function P = endmember1(Z,Y,Po,A,D,V,rho,parallel)
