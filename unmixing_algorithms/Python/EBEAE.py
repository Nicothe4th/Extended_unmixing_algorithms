...
def endmember(Y, A, rho):
    ...
    L = Y.shape[0]
    P = np.zeros((L, N))
    R = np.sum(N - np.arange(1, N))  # Equivalent to sum(N - (1:(N-1))) in MATLAB

    # Compute weight matrix W correctly
    W = (1.0 / K / np.sum(Y ** 2, axis=0)).reshape(1, -1)  # Shape: (1, K)
    
    if Y.shape[1] != K:
        raise ValueError("ERROR: the number of columns in Y and A does not match")

    # Define matrices O, n1, m1
    O = N * np.eye(N) - np.ones((N, N))
    n1 = np.ones((N, 1))
    m1 = np.ones((L, 1))

    # Apply W as a diagonal matrix
    T0 = A @ (np.diag(W.flatten()) @ A.T) + rho * (O / R)
    
    # Ensure numerical stability
    while 1 / np.linalg.cond(T0) < 1e-6:  # Matches MATLAB’s rcond(T0) < 1e-6
        rho /= 10
        T0 = A @ (np.diag(W.flatten()) @ A.T) + rho * (O / R)
        if rho < 1e-6:
            raise ValueError("Unstable numerical results in end-members estimation, update rho!!")

    # Use np.linalg.solve instead of np.linalg.inv for better stability
    Q = np.linalg.solve(T0, np.eye(N))
    
    # Compute end-members matrix
    T2 = Y @ (np.diag(W.flatten()) @ A.T) @ Q
    ...
    # Normalize Optimal Solution (avoid division by zero)
    P_sum = np.sum(P_est, axis=0)
    P_sum[P_sum == 0] = 1  # Prevent division by zero
    P = P_est / P_sum

    return P

def ebeae(Yo=[], N=2, parameters=[], Po=[], oea = 0):
    initcond = 1
    rho = 0.1
    Lambda = 0
    epsilon = 1e-3
    ...
    while (Jp-J)/Jp >= epsilon and iter<=maxiter  and oea==0:
        Am = abundance(Ym,P,Lambda,parallel);
        Pp = P.copy();
        P = endmember(Ym,Am,rho);
        Jp = J
        residual = Ym-P@Am
    ...
    if downsampling == 0:
        ...
        Ymos = np.c_[Ymo[:, Ins]]
        Ams = abundance(Ymos, P, Lambda, parallel)
        A = np.hstack((Am, Ams))
        II = np.hstack((Is, Ins))
