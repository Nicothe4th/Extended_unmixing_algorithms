...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 24 13:23:52 2025
...
def abundance(Y,P,W,Lambda,parallel=False):
    """
    ...
    Go = P.T @ P
    em = np.eye(N)
    lmin = np.min(np.linalg.svd(Go)[1]) * Lambda 
    G = Go + em * lmin  
...
def ebeaetv(Yo=[],N=2,parameters=[],Po=[],oae=0):
    L,K = Yo.shape
    initcond=1;
    rho=0.1;
    Lambda=1e-4;
    ...
    if display==1:
            print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
            print('EBEAE, Linear Unmixing & Total Variarion');
